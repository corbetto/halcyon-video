...
## Chain apparel correction

The crown stands above a horizontal sweatband; a gently drooping visor projects forward from its lower rim. The visor and garment trim take the active secondary color, while shirt and crown use primary. Neutral cloth maps preserve the weave under runtime palette dyes. The active LogoSpec is drawn with drawLogo and projected onto the actual shirt mesh with DecalGeometry, so the print follows the folds. Brand changes refresh dyes and print together; fixture teardown removes the extra decal geometry, material, texture and listener. The source remains an original garment design rather than a measured historical replica. Updated exported bounds and costs are in counter-apparel-metrics.json.
